...
def print_banner():
    ...
    if HAS_FIGLET:
        # Пробуем шрифты по приоритету — roman лучший, но может отсутствовать
        for font_name in ["roman", "colossal", "larry3d", "big"]:
            try:
                title = pyfiglet.figlet_format("ARGUS", font=font_name)
                break
            except Exception:
                continue
        else:
            title = pyfiglet.figlet_format("ARGUS")
    else:
        title = (
    ...
    try:
        ...
        for tx in txs_raw[:10]:
            ...
            txs.append({
                ...
                "is_contract": False,  # Bitcoin не имеет контрактов
                "from":   "",
                "to":     "",
            })
        result["txs"] = txs
...
KNOWN_ENTITIES = {
    ...
    "0x28c6c06298d514db089934071355e5743bf21d60": ("Binance Hot 2",      "exchange", "low"),
    "0x742d35cc6634c0532925a3b844bc454e4438f44e": ("Bitfinex",           "exchange", "low"),
    "0x77134cbc06cb00b66f4c7e623d5fdbf6777635ec": ("Coinbase",           "exchange", "low"),
    "0xa9d1e08c7793af67e9d92fe308d5697fb81d3e43": ("Coinbase Prime",     "exchange", "low"),
    "0x6cc5f688a315f3dc28a7781717a9a798a59fda7b": ("OKX Hot Wallet",     "exchange", "low"),
    "0x236f9f97e0e62388479bf9e5ba4889e46b0273c3": ("Bybit",              "exchange", "low"),
    "0x46340b20830761efd32832a74d7169b29feb9758": ("Kraken",             "exchange", "low"),
    "0x2910543af39aba0cd09dbb2d50200b3e800a63d2": ("Kraken 2",           "exchange", "low"),
    "0x4fabb145d64652a948d72533023f6e7a623c7c53": ("Binance BUSD",       "stablecoin","low"),
    ...
    "0x8589427373d6d84e98730d7795d8f6f8731fda16": ("Tornado Cash",       "mixer",    "high"),
    "0x905b63fff465b9ffbf41dea908ceb12478ec7601": ("Tornado Cash",       "mixer",    "high"),

    # ── Known Persons ────────────────────────────────────────────────────
    "0xd8da6bf26964af9d7eed9e03e53415d37aa96045": ("Vitalik Buterin",    "person",   "low"),
    "0xab5801a7d398351b8be11c439e05c5b3259aec9b": ("Vitalik Buterin 2",  "person",   "low"),
    "0x220866b1a2219f40e72f5c628b65d54268ca3a9d": ("CZ Binance",         "person",   "low"),

    # ── Foundations / DAOs ───────────────────────────────────────────────
    "0xde0b295669a9fd93d5f28d9ec85e40f4cb697bae": ("Ethereum Foundation","foundation","low"),
    "0xf835a0247b0063c04ef22006ebe57c5f11977cc4": ("Ethereum Foundation 2","foundation","low"),
    "0xebe4a49df7885d015329c919bf43e6460a858f1e": ("Ethereum Foundation 3","foundation","low"),
    "0x1db3439a222c519ab44bb1144fc28167b4fa6ee6": ("Vitalik — Gitcoin",  "donation", "low"),
}
...
# Пользовательская база адресов — загружается из custom_labels.json
_custom_labels = None

def load_custom_labels():
    """
    Загружает пользовательскую базу адресов из custom_labels.json.
    Формат файла:
    {
      "0xADDRESS": {"name": "ФБК Навального", "category": "ngo", "risk": "low"},
      "1BTC_ADDRESS": {"name": "Satoshi Reserve", "category": "historic", "risk": "low"}
    }
    """
    global _custom_labels
    if _custom_labels is not None:
        return _custom_labels
    
    custom_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "custom_labels.json")
    ...
    try:
        with open(custom_path, "r", encoding="utf-8") as f:
            ...
            _custom_labels = {k.lower(): v for k, v in raw.items()}
            return _custom_labels
    except Exception as e:
        _custom_labels = {}
        return _custom_labels
...
def resolve_address_label(address, label_db=None):
    ...
    Порядок: custom_labels → local known_entities → label_db → blockscout → etherscan
    Возвращает dict {name, category, source} или None
    """
    addr_low = address.lower()

    # 0. Пользовательская база (высший приоритет)
    custom = load_custom_labels()
    if addr_low in custom:
        info = custom[addr_low]
        return {
    ...
    try:
        ...
        if r.status_code == 200:
            data = r.json()
            # Public tags (e.g. "Vitalik Buterin")
            pub_tags = data.get("public_tags") or []
            if pub_tags:
                tag_name = pub_tags[0].get("label") or pub_tags[0].get("name") or ""
                if tag_name:
                    return {"name": tag_name, "category": "public tag", "risk": "low", "source": "blockscout"}
            # Private tags / watchlist
            priv_tags = data.get("private_tags") or []
            if priv_tags:
                tag_name = priv_tags[0].get("label") or ""
                if tag_name:
                    return {"name": tag_name, "category": "private tag", "risk": "low", "source": "blockscout"}
            # Name from Blockscout ENS/contract
            name = data.get("name") or ""
            if name:
                is_contract = data.get("is_contract", False)
                return {"name": name, "category": "contract" if is_contract else "entity",
                        "risk": "low", "source": "blockscout"}
    except Exception:
        pass

    # 4. Etherscan contract name (только для EVM)
...
def resolve_btc_label(address):
    """BTC attribution: custom → локальная база → WalletExplorer API."""
    custom = load_custom_labels()
    if address.lower() in custom:
        info = custom[address.lower()]
        return {"name": info.get("name","?"), "category": info.get("category","custom"),
                "risk": info.get("risk","low"), "source": "custom"}
...
def resolve_trx_label(address, tronscan_data=None):
    """TRX attribution: custom → локальная база → Tronscan."""
    custom = load_custom_labels()
    if address.lower() in custom:
        info = custom[address.lower()]
        return {"name": info.get("name","?"), "category": info.get("category","custom"),
...
def resolve_sol_label(address):
    """SOL attribution: custom → локальная база → Solana FM → Solscan."""
    custom = load_custom_labels()
    if address.lower() in custom:
        info = custom[address.lower()]
        return {"name": info.get("name","?"), "category": info.get("category","custom"),
...
def resolve_ton_label(address):
    """TON attribution: custom → локальная база → TON API."""
    custom = load_custom_labels()
    if address.lower() in custom:
        info = custom[address.lower()]
        return {"name": info.get("name","?"), "category": info.get("category","custom"),
...
def check_known_entities(address, txs):
    findings = []
    addr_low = address.lower()
    # Собираем только контрагентов, не сам адрес
    all_addrs = set()
    for tx in txs:
        if tx.get("from") and tx["from"].lower() != addr_low:
            all_addrs.add(tx["from"].lower())
        if tx.get("to") and tx["to"].lower() != addr_low:
            all_addrs.add(tx["to"].lower())
...
def print_single_chain_report(address, data, ofac=None):
    ...
    if txs:
        ...
        for tx in txs[:7]:
            val = tx["value"]
            if tx.get("is_contract"):
                val_str = f"{DIM}contract call{RS}"
            elif val == 0:
                val_str = f"{DIM}0.0 {ticker}{RS}"
            else:
                val_str = f"{W}{format_val(val, ticker)}{RS}"
            # Направление если есть from/to
            addr_low = address.lower()
            if tx.get("to") and tx.get("from"):
                direction = f"{G}IN {RS}" if tx.get("to","").lower() == addr_low else f"{Y}OUT{RS}"
                dir_str = f"  {direction}"
            else:
                dir_str = "      "
            status_c = G if tx["status"] in ("success", "confirmed") else R
            print(f"  {DIM}{tx['date']}{RS}{dir_str}  {val_str}  {status_c}[{tx['status']}]{RS}")
        print()
